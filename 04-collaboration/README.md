...
When you wish to contribute to a project, the first step is to create a remote copy of the original repository you wish to contribute to.

Instead of cloning the repository first, GitHub has a feature known as `forking` where you create a copy of the original repository (owned by someone else) in your own GitHub account. This will be a remote copy of the original repository.
...
After this, everything else is left up to the owner. They are free to review the changes proposed and comment on them. GitHub supports a discussion forum per pull request so that others can chime in about the changes and further improvements can be made before merging them into the original repository.
...
4. In GitHub, create a pull request where the `base` is the original repository's `master` branch and the `compare` is your copy of the repository's `master` branch. Optionally, fill in the information about the pull request
